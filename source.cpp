...
void TC() {
    ...
    // Ev of ball type
    mint s_ev = 0, n_ev = 0, T = 0;
    for(int i = 0; i < k; i++) s_ev += a[i], T += a[i];
    for(int i = k; i < n; i++) n_ev += a[i], T += a[i];
    s_ev /= k;
    if (n - k) n_ev /= (n - k);
    ...
    mint p_odd, ev_send = 0, ev_nend = 0;
    // What is ev of this?
    ev_nend += n_ev;
    ...
    ev_nend += s_ev * E_s;
    // This is with a special at the end
    // What is ev of this?
    E_s = 0;
    p_s = 1;
    for(int i = 1; i <= k; i++) {
    ...
    mint p_send = k;
    p_send /= n;
    mint A = ev_nend * anc + p_send * ev_send;
