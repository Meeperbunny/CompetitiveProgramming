...
void TC() {
    ...
    mint currProd = 1;
    for(int i = 0; i < n; ++i) {
        currProd *= min(a[i], b[i]);
    }
    map<int, int> cntA, cntB;
    map<int, int> topOfA, topOfB;
    for(int i = 0; i < n; ++i) {
        cntA[a[i]] += 1;
        cntB[b[i]] += 1;
        topOfA[a[i]] = max(i, topOfA[a[i]]);
        topOfB[b[i]] = max(i, topOfB[b[i]]);
    }
    cout << currProd << ' ';
    while(q--) {
        int o, x;
        ...
        if (o == 1) {
            ...
            oa[x] = nu;
            int ind = topOfA[old];
            int oldMin = min(a[ind], b[ind]);
            cntA[old]--;
            cntA[nu]++;
            // if (cntA[old] == 0) topOfA.erase(topOfA.find(old));
            a[ind] = nu;
            topOfA[old]--;
            topOfA[nu] = max(ind, topOfA[nu]); 
            int newMin = min(a[ind], b[ind]);
            currProd /= oldMin;
            currProd *= newMin;
        }
        else if (o == 2) {
            ...
            cntB[old]--;
            cntB[nu]++;
            // if (cntB[old] == 0) topOfB.erase(topOfB.find(old));
            b[ind] = nu;
            topOfB[old]--; 
            topOfB[nu] = max(ind, topOfB[nu]);
            int newMin = min(a[ind], b[ind]);
            currProd /= oldMin;
            currProd *= newMin;
        }
