...
void TC() {
    ...
    // Has to have 1.
    // stars and balls?
    // Split n - 1 into k - 1 groups
    // Ending at n? How many ways
    mint tot = 0;
    for(int mult = 1; mult <= n; mult++) {
        int n_end = n - (n % mult) + mult;
        // for(int end = k * mult; end <= n_end; end += mult) {
        //     const int n_n = (end - mult) / mult;
        //     const int n_k = k - 1;
        //     tot += combo.choose(n_n - 1, n_k - 1);
        // }
        const int n_n = (n_end - mult) / mult;
        const int n_k = k;
        // cout << mult << ' ' << n_end << ' ' << combo.choose(n_n - 1, n_k - 1)<< endl;
        tot += combo.choose(n_n - 1, n_k - 1);
    }
    cout << tot << endl;
}
