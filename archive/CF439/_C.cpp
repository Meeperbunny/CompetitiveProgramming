...
void TC() {
    ...
    for(int i = 0; i <= min(a, b); i++) {
        ab += combo.choose(a, i) * combo.factorial(b) / combo.factorial(b - i);
    }
    mint bc = 0;
    for(int i = 0; i <= min(b, c); i++) {
        bc += combo.choose(b, i) * combo.factorial(c) / combo.factorial(c - i);
    }
    mint ca = 0;
    for(int i = 0; i <= min(c, a); i++) {
        ca += combo.choose(c, i) * combo.factorial(a) / combo.factorial(a - i);
    }
    ways = ab * bc * ca;
    cout << ways << endl;
}
