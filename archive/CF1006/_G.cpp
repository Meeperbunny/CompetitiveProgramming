...
void TC() {
    ...
    cin >> n >> k;
    mint tot = 0;
    // Idea is manually calc bases 2 to 1000.
    // Then, since will be 2 digits, calculate the sum of base 1000 to floor{n^0.5}.
    // All bases after will be n.
    ll top = 1000;
    for(ll b = 2; b <= min({top, k, n}); b++) {
        ll sz = 0;
        ll tn = n;
        while(tn) {
            ll nxt = tn / b;
    ...
    if (n >= top+1) {
        for (ll leftDig = 1; leftDig <= 1000; leftDig++) {
            ll lowerBase = max(top+1, ll((n + 1 + leftDig) / (leftDig + 1)));
            ll upperBase = min(k, ll(n / leftDig));
            ll cnt = upperBase - lowerBase + 1;
            if (cnt <= 0) continue;
            ll baseSum = leftDig * cnt;
            
            ll r = n - lowerBase * leftDig;
            auto sum = [](ll r, ll b, ll c, ll j) -> ll {
                return c * b * r 
                       + (r - b * j) * (c - 1) * c / 2 
                       - j * (c - 1) * c * (2 * c - 1) / 6;
            };
            mint toAdd = baseSum;
            toAdd += sum(r, lowerBase, cnt, leftDig);
            tot += toAdd;
        }
    }
    // Given this range of bases, what is the left? sum of n * cnt - bases sum * left num 
    // TODO
    // For k > n, will just be n.
    if (k > n) {
        mint res = k - n;
        tot += mint(res * n);
    }
