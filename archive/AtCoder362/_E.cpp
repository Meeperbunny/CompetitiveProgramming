...
void TC() {
    ...
    vl A(N);
    GETV(A);
    vector<map<LP, mint>> K(N + 1);
    FOR(i, N) {
        dbg(A[i]);
        LL curr = A[i];
        for(int k = N - 1; k >= 1; k--) {
            for(auto c : K[k]) {
                if (c.first.first + c.first.second == curr) {
                    LP newPair = {curr, c.first.second};
                    K[k + 1][newPair] += c.second;
                }
            }
        }
        for(auto e : K[0]) {
            LP newPair = {curr, curr - e.first.first}; 
            mint cnt = e.second;
            dbg(cnt);
            K[1][newPair] += cnt;
        }
