...
void TC() {
    ...
    for(int i = 0; i <= 500; i++) for(int q = 0; q <= 500; q++) {
        dp[i][q] = 0, prefix[i][q] = 0;
    }

    for(int i = 1; i <= n; i++) {
        for(int q = 0; q <= i; q++) {
            int j = pow(2, i) - 1;
            prefix[i][q] = combo.choose(j + q - 1, q);
            if (0 == prefix[i][q]) prefix[i][q] = 1;
        }
    }

    // for(int i = 0; i <= n; i++) {
    //     for(int q= 0; q <= n; q++) cout << prefix[i][q] << ' ';
    //     cout << endl;
    // }
    for(int i = 1; i <= n; i++) {
        for(int q = 1; q <= i; q++) {
            prefix[i][q] += prefix[i][q - 1];
        }
    }
    ...
    for(int i = 2; i <= n; i++) {
        for(int ops = 0; ops <= k; ops++) {
            for(int l = 0; l <= ops; l++) {
                int used = max(ops - l, l);
                int left = ops - used;
                // dbg(ops - l, l);
                // dbg(used, left);
                int of = l * 2 == ops;
                // dbg(i, ops, dp[i - 1][used], prefix[i - 1][left - of]);
                dp[i][ops] += dp[i - 1][used] * prefix[i - 1][left - of] * int(of + 1);
            }
        }
    }
    // for(int i = 0; i <= n; i++) {
    //     for(int q = 0; q <= k; q++) cout << dp[i][q] << ' ';
    //     cout << endl;
    // }
    cout << dp[n][k] << endl;
}
