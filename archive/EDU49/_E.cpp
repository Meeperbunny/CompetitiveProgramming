...
void TC() {
    ...
    for(int sz = 1; sz < n; sz++) {
        ...
        for(int mv = 1; mv < n; mv++) {
            for(int lcon = 1; lcon < n; lcon++) {
                for(int p = 0; p < 2; p++) {
                    nx[max(mv, lcon + 1)][lcon + 1][p] += la[mv][lcon][p];
                    nx[mv][1][1 - p] += la[mv][lcon][p];
                }
            }
        }
        swap(nx, la);
    ...
    // For each I, count how many ways you can extend given total current area. Given consec, transition
    mint ans = 0;
    for(int lcon = 1; lcon <= n; lcon++) {
        if (lcon >= k) continue;
        // Given current size, consec, find ways.
        ...
        for(int sz = 1; sz < n; sz++) {
            for(int cc = 1; cc < n; cc++) {
                // Check that in area
                if ((cc + 1) * lcon < k)
                    sdp[sz + 1][cc + 1] += sdp[sz][cc];
                sdp[sz + 1][1] += sdp[sz][cc];
            }
        }
        for(int i = 1; i <= n; i++) {
            ans += sdp[n][i];
